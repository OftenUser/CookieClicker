...
if (1 == 1 || undefined == Math.seedrandom) {
	...
	(function(a, b, c, d, e, f) {
		...
		function l(a, b) {
			...
			if (b && "o" == d) for (e in a) try {c.push(l(a[e], b - 1))} catch (f) {} return c.length ? c : "s" == d ? a : a + "\0"
		}
		function m(a, b) {
		...
		c.seedrandom = function(a, f) {
			...
			return m(o(q.S), b), c.random = function() {
				...
				for(; a >= i;) a /= 2, b /= 2, c >>>= 1;
				return (a + c) / b}, p}, m(c.random(), b)
		})(this, [], Math, 256, 6, 52);
	}

	if (1 == 1 || undefined == choose) {
		function choose(arr) {
			if (arr.length == 0) return 0; else return arr[Math.floor(Math.random() * arr.length)];
		}
	}
